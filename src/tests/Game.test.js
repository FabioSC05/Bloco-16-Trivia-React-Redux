...
describe('Testar a tela Game.js', () => {
  test('Se redireciona para tela de login quando token é inválido', async () => {
    ...
    await waitFor(() => expect(screen.getByTestId('input-player-name').value).toBe(""));
    await waitFor(() => expect(global.fetch).toHaveBeenCalled());
    expect(history.location.pathname).toBe('/');

    global.fetch.mockClear();
  })
  test('Se os botões são desabilitados ao zerar o timer', async () => {
    ...
    const timer = screen.getByText(/Tempo/);
    expect(timer).toBeInTheDocument();
    expect(timer.innerHTML).toBe('Tempo: 30');
    expect(correctButton).toBeInTheDocument();
    expect(correctButton).toBeEnabled();
    await waitFor(() => expect(timer.innerHTML).toBe('Tempo: 0'), { timeout: 35000 });
  ...
  test('Se score de jogador é renderizado corretamente quando acerta as questões com tempo 2s', async () => {
    ...
    await waitFor(() => expect(global.fetch).toHaveBeenCalled());
    const headerScore = screen.getByTestId('header-score');
    expect(headerScore.textContent).toBe('0');
    global.fetch.mockClear();
  }, 10000)
})
